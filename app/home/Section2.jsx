    ...
    <section className="px-[1.2rem] md:px-[4rem] pt-14 pb-16 md:pb-20 flex items-center flex-col bg-[#30304C] text-white relative">
      ...
      <div className="flex flex-wrap justify-center gap-8 md:gap-16 xl:gap-20 mt-10 md:mt-16">
        {features.map((feature) => (
          ...
          >
            <img src={feature.imageUrl} alt="#" className="w-[5rem] md:w-[9rem] h-[5rem] md:h-[9rem]" />
            <h3 className="text-[18px] md:text-[24px] font-medium mt-3 md:mt-6">{feature.title}</h3>
            <p className="text-[13px] md:text-[15px] mt-3">{feature.description}</p>
          </div>
        ))}
      </div>
    </section>
